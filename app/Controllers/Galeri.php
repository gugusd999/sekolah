...
$child[] = $value->tanggal; 
            ...
            $child[] = "
                <center>
                    <a href='" . site_url('Galeri/edit/'.$value->id) . "' class='btn btn-sm btn-warning'>Edit</a>
    ...
    public function Index($edit = "")
    {
        if(isset($_POST['data'])){
            ...
            $data['form'] = $this->form();
            return redirect()->to('/Galeri'); // ubah disini
        }else{
            $data['form'] = $this->form();
    ...
    {
        $data['edit'] = $this->db->query("SELECT * FROM Galeri WHERE id = '$edit' ")->getRow();
        $data['form'] = $this->form();
        return view('admin/Galeri/index', $data);
    }
    ...
    public function simpan()
    {
        $data = $_POST['data'];
        $this->builder->insert($data);
        return redirect()->to('/form-Galeri');
    }

    public function formUpdate()
    {
        return view('admin/Galeri/update');
    }

    public function hapus()
    {
        $kode = $_POST['id'];
        $this->db->query("DELETE FROM Galeri WHERE id = '$kode' ");
        return redirect()->to('/Galeri'); // ubah disini
    }
}
