...
$child[] = $value->nama; 
            ...
            $child[] = "
                <center>
                    <a href='" . site_url('mAgama/edit/'.$value->id) . "' class='btn btn-sm btn-warning'>Edit</a>
    ...
    public function Index($edit = "")
    {
        if(isset($_POST['data'])){
            ...
            $data['form'] = $this->form();
            return redirect()->to('/mAgama'); // ubah disini
        }else{
            $data['form'] = $this->form();
            return view('admin/mAgama/index', $data);
        }
    }
    ...
    {
        $data['edit'] = $this->db->query("SELECT * FROM mAgama WHERE id = '$edit' ")->getRow();
        $data['form'] = $this->form();
        return view('admin/mAgama/index', $data);
    }
    ...
    public function simpan()
    {
        $data = $_POST['data'];
        $this->builder->insert($data);
        return redirect()->to('/form-mAgama');
    }

    public function formUpdate()
    {
        return view('admin/mAgama/update');
    }

    public function hapus()
    {
        $kode = $_POST['id'];
        $this->db->query("DELETE FROM mAgama WHERE id = '$kode' ");
        return redirect()->to('/mAgama'); // ubah disini
    }
}
